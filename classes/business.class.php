...
{ 
		...
		{ 
			// Generating the time drop down menu
				...
				$leftspace = $_SESSION['outlet_max_capacity']-$availability[date('H:i',$value)];
				echo $txt_value;
				if ($showtime == 1) {
					echo " - ".$leftspace." "._seats." "._free;
				}
				echo"</option>\n";
...
function reservationsByTime() {
	
	$availability_by_time = array();
	$availability =	querySQL('availability');
	if ($availability) {
		foreach($availability as $row) {
			$availability_by_time[$row->reservation_time] =$row->pax_total;
		}
	}
	return $availability_by_time;
}
